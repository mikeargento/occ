...
Proxy installed: !`command -v occ-mcp-proxy >/dev/null 2>&1 && echo "yes" || (npm ls -g occ-mcp-proxy 2>/dev/null | grep occ-mcp-proxy || echo "no")`
...
### `/proofstudio:occ-agent` or `/proofstudio:occ-agent start` — Start the proxy
...
6. Show them how to manage agents at `https://proofstudio.xyz/agent`.

### `/proofstudio:occ-agent setup` — Full setup with Claude Desktop
...
### `/proofstudio:occ-agent status` — Check status
...
### `/proofstudio:occ-agent stop` — Stop the proxy
