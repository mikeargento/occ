...
type OccSprite = (typeof OCC_SPRITES)[number];
...
function spriteSize(sprite: OccSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = OCC_SPRITES[(baseCol + baseRow) % OCC_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = OCC_SPRITES[Math.floor(Math.random() * OCC_SPRITES.length)]!;
