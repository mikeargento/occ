...
Use when:
- You want OCC Agent to run Cursor Agent CLI locally as the agent runtime
...
- OCC Agent auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$paperclip" and related skills on local runs.
- OCC Agent auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
