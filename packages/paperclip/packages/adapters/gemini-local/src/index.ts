...
Use when:
- You want OCC Agent to run the Gemini CLI locally on the host machine
...
- Gemini CLI is not installed on the machine that runs OCC Agent
...
- OCC Agent auto-injects local skills into \`~/.gemini/skills/\` via symlinks, so the CLI can discover both credentials and skills in their natural location.
