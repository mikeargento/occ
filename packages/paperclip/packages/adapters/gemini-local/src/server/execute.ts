...
function renderApiAccessNote(env: Record<string, string>): string {
  ...
  return [
    "OCC Agent API access note:",
    "Use run_shell_command with curl to make OCC Agent API requests.",
    "GET example:",
...
/**
 * Inject OCC Agent skills directly into `~/.gemini/skills/` via symlinks.
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const promptTemplate = asString(
    ...
    "You are agent {{agent.id}} ({{agent.name}}). Continue your OCC Agent work.",
  );
