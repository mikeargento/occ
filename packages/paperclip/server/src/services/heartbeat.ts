...
import { signPreExec, signPostExec, signEvent } from "./occ-signer.js";
...
export function heartbeatService(db: Db) {
  ...
  async function appendRunEvent(
    ...
    // OCC: sign event (non-blocking, best-effort)
    const occProof = process.env.OCC_SIGN_EVENTS !== "false"
      ? await signEvent({
          runId: run.id,
          agentId: run.agentId,
    ...
    await db.insert(heartbeatRunEvents).values({
      ...
      occProof: occProof as unknown as Record<string, unknown> | undefined,
    });
  ...
  async function executeRun(runId: string) {
    ...
    try {
      ...
      // OCC: sign pre-execution manifest
      const preExecProof = await signPreExec({
        runId: run.id,
        agentId: agent.id,
      ...
      if (preExecProof) {
        await db.update(heartbeatRuns)
          .set({ occProofPreExec: preExecProof as unknown as Record<string, unknown> })
          .where(eq(heartbeatRuns.id, run.id));
      }
      ...
      // OCC: sign post-execution result
      const postExecProof = await signPostExec({
        runId: run.id,
        agentId: agent.id,
        companyId: agent.companyId,
        status,
        ...
        logSha256: logSummary?.sha256,
        logBytes: logSummary?.bytes,
        model: adapterResult.model ?? undefined,
        provider: adapterResult.provider ?? undefined,
        costUsd: adapterResult.costUsd,
      });
      if (postExecProof) {
        await db.update(heartbeatRuns)
          .set({ occProofPostExec: postExecProof as unknown as Record<string, unknown> })
          .where(eq(heartbeatRuns.id, run.id));
      }
