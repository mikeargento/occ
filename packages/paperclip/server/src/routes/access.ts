...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "OCC Agent is bound to loopback in authenticated/private mode.",
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # OCC Agent OpenClaw Gateway Onboarding
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested OCC Agent base URLs to try");
