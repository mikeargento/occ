...
import { signPolicyChange } from "../services/occ-signer.js";
...
export function agentRoutes(db: Db) {
  ...
  router.patch("/agents/:id", validate(updateAgentSchema), async (req, res) => {
    ...
    // OCC: when occPolicy changes, commit it as a cryptographic proof
    // UI sends occPolicy nested inside metadata, so check both locations
    const patchMeta = (patchData as Record<string, unknown>).metadata as Record<string, unknown> | undefined;
    const occPolicyPayload = (patchMeta?.occPolicy ?? (patchData as Record<string, unknown>).occPolicy) as Record<string, unknown> | undefined;
    if (occPolicyPayload) {
      void signPolicyChange({
        agentId: agent.id,
        agentName: agent.name,
        companyId: agent.companyId,
        occPolicy: occPolicyPayload,
