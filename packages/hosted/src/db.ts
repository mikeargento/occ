...
export const db = {
  ...
  async upsertUser(user: { id: string; email: string; name: string; avatar: string; provider: string; providerId: string; mcpToken: string }) {
    const p = getPool();
    await p.query(
      `INSERT INTO users (id, email, name, avatar, provider, provider_id, mcp_token)
  ...
  async addProof(userId: string, entry: { agentId: string; tool: string; allowed: boolean; args?: unknown; output?: unknown; reason?: string; proofDigest?: string; receipt?: unknown }) {
    const p = getPool();
    await p.query(
      `INSERT INTO proofs (user_id, agent_id, tool, allowed, args, output, reason, proof_digest, receipt)
  ...
  async getProofs(userId: string, limit = 50, offset = 0) {
    ...
    const res = await p.query(
      "SELECT * FROM proofs WHERE user_id = $1 ORDER BY created_at DESC LIMIT $2 OFFSET $3",
      [userId, limit, offset]
    );
  ...
  async upsertAgent(userId: string, agent: { id: string; name: string; allowedTools?: string[] }) {
    const p = getPool();
    await p.query(
      `INSERT INTO agents (id, user_id, name, allowed_tools)
       VALUES ($1, $2, $3, $4)
  ...
  async incrementAgentCalls(userId: string, agentId: string, allowed: boolean) {
    ...
    await p.query(
      `UPDATE agents SET total_calls = total_calls + 1, ${col} = ${col} + 1 WHERE user_id = $1 AND id = $2`,
      [userId, agentId]
    );
  },
  ...
  async createPolicy(userId: string, policy: { name: string; allowedTools: string[]; maxActions?: number; rateLimit?: string; policyDigest?: string }) {
    ...
    const res = await p.query(
      `INSERT INTO policies (user_id, name, allowed_tools, max_actions, rate_limit, policy_digest)
