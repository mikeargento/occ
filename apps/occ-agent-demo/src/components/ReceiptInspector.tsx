...
export function ReceiptInspector({ result, onBack }: ReceiptInspectorProps) {
    ...
    return (
      <div className="space-y-4">
        <div className="rounded-xl border border-border-subtle bg-bg-elevated flex flex-col items-center py-14 px-6">
          ...
          <div className="text-xs text-text-tertiary mt-1">Run a tool to generate an execution receipt.</div>
        </div>
        <button
          onClick={onBack}
  ...
  const counter = proof?.commit ? (proof.commit as Record<string, unknown>).counter : null;
    ...
    <div className="space-y-4">
      ...
      <div className={`rounded-xl border p-5 animate-slide-up ${
        ...
        <div className="flex items-start gap-4">
          <div className={`w-10 h-10 rounded-xl border flex items-center justify-center shrink-0 ${
            hasProof
              ? "bg-success/10 border-success/20 animate-success-pulse"
              : "bg-warning/10 border-warning/20"
          }`}>
            {hasProof ? (
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-success">
                <path d="M4 9l4 4L14 5" />
              </svg>
            ) : (
              <svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="text-warning">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[15px] font-semibold text-text">
              {hasProof ? "Execution committed" : "Executed without proof"}
            </div>
            <p className="text-sm text-text-tertiary mt-0.5">
              {hasProof ? (
                <>
                  <span className="font-mono">{env.tool}</span>
                  {" · "}
                  <span className="font-mono">{result.output.status}</span>
                  {" · "}
                  <span className="font-mono">{result.output.fetchDuration}ms</span>
                  {counter != null && (
                    <> {" · "} <span className="text-success font-mono">#{String(counter)}</span></>
                  )}
                </>
              ) : (
                <>Tool ran successfully but OCC commit was not available.</>
              )}
            </p>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-border-subtle bg-bg-elevated">
        <div className="px-5 py-3.5 border-b border-border-subtle flex items-center justify-between">
          <span className="text-[11px] font-semibold uppercase tracking-[0.15em] text-text-tertiary">Response</span>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] text-text-tertiary font-mono">{result.output.fetchDuration}ms</span>
          </div>
        </div>
        <div className="px-5 py-4 space-y-2.5">
          <MetaRow label="URL" value={result.output.url} />
          <MetaRow label="Content-Type" value={result.output.contentType} />
          <MetaRow label="Size" value={`${result.output.contentLength.toLocaleString()} bytes`} />
        </div>
      ...
      >
        ...
        <div className="mt-4 pt-4 border-t border-border-subtle">
          <div className="flex items-baseline justify-between mb-2">
            <div className="text-[11px] text-text-tertiary uppercase tracking-wide">Committed Digest</div>
            <CopyButton value={result.envelopeDigestB64} />
          </div>
          <div className="text-sm font-mono text-accent break-all leading-relaxed">
            {result.envelopeDigestB64}
          </div>
          <p className="text-[11px] text-text-tertiary mt-2">
            SHA-256 of the canonical envelope. This is the only value sent to OCC.
          </p>
        </div>
      </ExpandableSection>
      ...
      <ExpandableSection
        ...
        badge={hasProof ? "signed" : "unavailable"}
        badgeColor={hasProof ? "success" : "warning"}
      >
        {proof ? (
          <div className="space-y-4">
            <div className="space-y-2.5">
          ...
          <div className="space-y-3">
            ...
            <p className="text-[11px] text-text-tertiary leading-relaxed">
              Set <span className="font-mono">OCC_API_URL</span> and <span className="font-mono">OCC_API_KEY</span> environment variables to connect to a commit service.
            </p>
          </div>
        )}
      </ExpandableSection>
      ...
      >
        ...
        <div className="mt-6 pt-4 border-t border-border-subtle space-y-4">
          <div>
            <div className="text-[11px] text-success uppercase tracking-wide font-semibold mb-2">This receipt proves</div>
            <ul className="text-[13px] text-text-secondary space-y-1.5 leading-relaxed">
              <li className="flex items-start gap-2">
                <span className="text-success mt-0.5 shrink-0">&#x2713;</span>
                The execution was committed through OCC
              </li>
              <li className="flex items-start gap-2">
                <span className="text-success mt-0.5 shrink-0">&#x2713;</span>
              ...
              <li className="flex items-start gap-2">
                <span className="text-success mt-0.5 shrink-0">&#x2713;</span>
                The commit occupies a unique position in the causal sequence
              </li>
            </ul>
          </div>
          <div>
      ...
      <div className="flex gap-3">
        ...
        <button
          onClick={() => {
            ...
            a.href = url;
            a.download = `receipt-${env.tool}-${counter ?? Date.now()}.json`;
            a.click();
    ...
    <div className="rounded-xl border border-border-subtle bg-bg-elevated terminal-glow overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2.5">
          <span>{title}</span>
          {badge && (
            <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded border ${badgeColors[badgeColor ?? "info"]}`}>
              {badge}
            </span>
          )}
        </div>
