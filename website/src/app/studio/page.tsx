...
1. PURPOSE
----------
This file is a self-contained verification guide for the proof package
created by ProofStudio (https://proofstudio.xyz).

It is designed so that any AI system (ChatGPT, Claude, Gemini, Grok, etc.)
can be given this file along with proof.json and the original artifact to
perform a complete, independent verification analysis.
...
------------------------
  ${filename}     — the original artifact (never uploaded; hashed locally)
...
The artifact was hashed locally in the user's browser. Only the digest
was sent to the commit service. The original file never left the device.
...
To verify the artifact matches the proof:
  ...
  1. Compute SHA-256 of the artifact file (${filename})
...
If they match, the file is the exact artifact this proof covers.
...
Signed body fields (these are covered by the signature):
  ...
  - measurement:         ${p.environment.measurement}${p.environment.attestation ? `\n  - attestationFormat:   ${p.environment.attestation.format}` : ""}${p.attribution ? `\n  - attribution:         (sealed — see below)` : ""}
...
Verification checks (all must pass):
  ...
  [B] Causal independence
      ...
      It contains NO artifact hash, NO digestB64 — proving it was created
      before any artifact was known
...
nonce-first behavior internally. The proof does not structurally demonstrate
the ordering to an external verifier.`}
`;
...
`;
  ...
  if (p.attribution) {
    txt += `
Attribution (sealed into the Ed25519 signed body):
${p.attribution.name ? `  Name:     ${p.attribution.name}\n` : ""}${p.attribution.title ? `  Title:    ${p.attribution.title}\n` : ""}${p.attribution.message ? `  Message:  ${p.attribution.message}\n` : ""}`;
  }
