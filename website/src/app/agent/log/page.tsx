...
import { getAuditLog } from "@/lib/agent/api";
import type { AuditEntry } from "@/lib/agent/types";
import { Card } from "@/components/agent/card";
import { Badge } from "@/components/agent/badge";
import { EmptyState } from "@/components/agent/empty-state";
import { formatTimestamp } from "@/lib/agent/format";
    ...
    <>
      ...
      {loading && (
        <div className="space-y-0">
          <div className="h-10 w-full rounded-t-xl bg-bg-elevated border border-border animate-pulse" />
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="h-12 w-full bg-bg-elevated border-x border-border animate-pulse" />
          ))}
        </div>
      )}
